...
function Others() {
  ...
  const handleFileUpload = (event) => {
    const files = Array.from(event.target.files).filter(
      file => !file.type.startsWith("image/") && !file.type.startsWith("video/") && !file.type.startsWith("application/")
    );
  ...
  const getFileTypeImage = () => fileTypeImages.other;
...
<div className="flex items-center ml-[40px]">
    ...
    <img src="/images/driveImage.svg" className='ml-[50px]' height={220} width={220} />
            ...
            <div className='bg-[#F2F4F8] w-[1127px] h-[752px] flex flex-col gap-5 rounded-3xl'>
              <h1 className='text-5xl font-bold ml-[50px] mt-[40px]'>Others</h1>
              ...
              <div className='flex flex-row w-[1070px] overflow-y-scroll h-[700px] mt-[20px] ml-[50px] flex-wrap gap-5'>
                {otherFiles.length > 0 ? (
                  ...
                  <p className="text-xl ml-4 text-gray-500">No other files uploaded yet.</p>
                )}
              </div>
            </div>
          </div>
